...
// Possiveis conteiners para uso do Algoritmo *A
#include <list>
#include <deque>
#include <set>
#include <queue>
...
double Labirinto::calculaCaminho(int &prof, int &NAbert, int &NFech)
{
	...
	// Se estiver vazio ou não tem inicio e fim
	...
	// Apaga um caminho anterior, caso haja
	limpaCaminho();
	...
	// Possui acesso randomico, sendo mais
	// facil de acessar a memória, em comparação a list
	// que deve ter um interacao do inicio->fim ou fim->inicio
	deque<Noh> Aberto;
	deque<Noh> Fechado;
	deque<Noh>::iterator oldA;
	deque<Noh>::iterator oldF;
					...
					if (dir != Coord(0, 0))
					{
						...
						if (movimentoValido(atual.pos, prox))
						{
							...
							if (oldF == Fechado.end() && oldA == Aberto.end())
							{
								...
								//Aumentou drasticamente em qualquer lugar que coloque
