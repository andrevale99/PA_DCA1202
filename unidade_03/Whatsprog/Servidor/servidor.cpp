...
void WhatsProgDadosServidor::send_buffer_msgs()
{
}
...
void WhatsProgDadosServidor::send_confirmacoes()
{
}
...
int WhatsProgDadosServidor::main_thread()
{
	...
	while (!fim)
	{
		...
		// todos os que estiverem conectados IMPLEMENTADO
		for (auto it_user : user)
		{
		...
		if (iResult == mysocket_status::SOCK_OK)
		{
			...
			for (auto it_user : user)
			{
				if (!fim && it_user.connected() && f.had_activity(it_user.getSocket()))
				{
					...
					if (it_user.connected())
						...
						case CMD_NOVA_MSG:
						{
							...
							if (iResult == mysocket_status::SOCK_OK)
							{
								iResult = it_user.read_string(destinatario, TIMEOUT_WHATSPROG * 1000);
							}
							if (iResult == mysocket_status::SOCK_OK)
							{
							...
							if (msg_valida && user.connected())
							{
								...
								if (envioOK)
									envioOK = (user.write_int(id) == mysocket_status::SOCK_OK);
								if (!envioOK)
								{
						...
						case CMD_MSG_LIDA1:
						{
							...
							if (iResult == mysocket_status::SOCK_OK)
								iResult = user.read_string(remetente, TIMEOUT_WHATSPROG * 1000);
							if (iResult != mysocket_status::SOCK_OK)
							{
								// Pode ser mysocket_status::SOCK_TIMEOUT, mysocket_status::SOCK_DISCONNECTED ou mysocket_status::SOCK_ERRO
								if (iResult == mysocket_status::SOCK_ERROR)
				...
				}		  // Fim do if (had_activity) no socket do clientes
			}
					...
					if (cmd == CMD_NEW_USER)
					{
						...
						for (auto it : user)
						{
							...
							/*if (it.connected())
							{
								...
								cerr << "Usuario (" << it.getLogin() << ") estah conectado no momento. Desconectando\n";
								t.write_int(CMD_LOGIN_INVALIDO);
								t.close();
							}*/
						}
