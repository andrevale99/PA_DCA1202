...
{
	if (definedPort(IdPort))
		return ports[IdPort - 1]->getId_in(IdPort);
...
void Circuit::digitar()
{
	...
	for (int i = 1; i <= getNumPorts(); ++i)
	{
		...
		if (tipo == "NT")
		{
			ports[i - 1] = new Port_NOT();
		}

		else if (tipo == "AN")
		{
			ports[i - 1] = new Port_AND();
		}

		else if (tipo == "NA")
		{
			ports[i - 1] = new Port_NAND();
		}

		else if (tipo == "OR")
		{
			ports[i - 1] = new Port_OR();
		}

		else if (tipo == "NO")
		{
			ports[i - 1] = new Port_NOR();
		}

		else if (tipo == "XO")
		{
			ports[i - 1] = new Port_XOR();
		}

		else if (tipo == "NX")
		{
			ports[i - 1] = new Port_NXOR();
		}
